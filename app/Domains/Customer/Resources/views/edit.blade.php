...
@section('title', 'Editar ' . $customer->name . ' - UPMANAGER')
...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <ol class="flex items-center space-x-4">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('customers.show', $customer->id) }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">
                        {{ $customer->name }}
                    </a>
                </div>
            </li>
    ...
    <div class="bg-white shadow-sm rounded-lg border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h1 class="text-2xl font-bold text-gray-900">{{ __('customers.edit_title') }}</h1>
            <p class="mt-1 text-sm text-gray-600">{{ __('customers.update_info') }}</p>
        </div>
        
        <form action="{{ route('customers.update', $customer->id) }}" method="POST" class="px-6 py-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                >
                    ...
                    <option value="active" {{ old('status', $customer->status) === 'active' ? 'selected' : '' }}>{{ __('customers.active') }}</option>
                    <option value="inactive" {{ old('status', $customer->status) === 'inactive' ? 'selected' : '' }}>{{ __('customers.inactive') }}</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
