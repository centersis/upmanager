    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <div class="mt-2 flex items-center space-x-4">
                ...
                <span class="text-sm text-gray-500">{{ __('customers.client_since') }} {{ $customer->created_at->format('d/m/Y') }}</span>
            </div>
        </div>
        <div class="flex items-center space-x-3">
            ...
            <form action="{{ route('customers.destroy', $customer->id) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('customers.confirm_delete') }}')">
                @csrf
                @method('DELETE')
                <button type="submit" 
        ...
        <div class="p-5">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 truncate">{{ __('customers.projects') }}</dt>
    ...
    <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 truncate">{{ __('customers.total_updates') }}</dt>
    ...
    <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 truncate">{{ __('customers.total_views') }}</dt>
...
<div class="bg-white shadow rounded-lg mb-8">
    <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-6">{{ __('customers.contacts') }}</h3>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        @if($contact->phone)
                            <div class="flex items-center mt-1">
                                ...
                                <a href="tel:{{ $contact->phone }}" class="text-xs text-gray-500 hover:text-blue-600 truncate">{{ $contact->phone }}</a>
                            </div>
                        @endif
                        @if($contact->email)
                            <div class="flex items-center mt-1">
                                ...
                                <a href="mailto:{{ $contact->email }}" class="text-xs text-gray-500 hover:text-blue-600 truncate">{{ $contact->email }}</a>
                            </div>
                        @endif
                    </div>
                </div>
...
<div class="bg-white shadow rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-6">{{ __('customers.customer_projects') }}</h3>
        ...
        <div class="mb-8 last:mb-0">
            <div class="border border-gray-200 rounded-lg p-6">
                ...
                <div class="mb-4">
                                                    <p class="text-sm text-gray-600">@choice('projects.update_count', $project->updates->count(), ['count' => $project->updates->count()])</p>
                </div>

                @if($project->updates->count() > 0)
                <div class="space-y-3">
                                            <h5 class="text-sm font-medium text-gray-900">{{ __('customers.recent_updates') }}:</h5>
                    ...
                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ $update->views }} {{ __('updates.views') }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="text-center py-4 text-gray-500">
                    <p class="text-sm">{{ __('updates.no_updates_for_project') }}</p>
                </div>
                @endif
        ...
        <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('customers.no_projects_found') }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ __('customers.no_projects') }}</p>
        </div>
        @endforelse
    </div>
</div>
...
function copyIframeCode(textareaId) {
    ...
    navigator.clipboard.writeText(iframeCode).then(function() {
        // Feedback visual
            ...
            notification.textContent = '{{ __('customers.iframe_copied') }}';
